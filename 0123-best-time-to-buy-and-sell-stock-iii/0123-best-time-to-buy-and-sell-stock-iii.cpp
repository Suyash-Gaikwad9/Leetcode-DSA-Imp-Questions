        ...
        for(int index = n-1; index >= 0; index--){
            for(int buy = 0; buy <= 1; buy++){
                for(int limit = 1; limit <= 2; limit++){
                    ...
                    if(buy){
                        int buyMove = -prices[index] + dp[index+1][0][limit];
                        int skipMove = 0 + dp[index+1][1][limit];
                        profit = max(buyMove, skipMove);
                    }
                    else{
                        int sellMove = +prices[index] + dp[index+1][1][limit-1];
                        int skipMove = 0 + dp[index+1][0][limit];
                        profit = max(sellMove, skipMove);
                    }
